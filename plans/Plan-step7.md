...
메인 메뉴 화면 상단에 표시한다. 시료가 하나도 없으면 "등록된 시료가 없습니다."로 표시한다.

메인 메뉴는 하위 컨트롤러 실행 중 시료가 새로 등록/변경될 수 있으므로, 요약 정보는 메뉴가
표시될 **때마다(최초 진입 시 및 하위 컨트롤러 종료 후 복귀 시 모두 포함)** `SampleModel`을
새로 생성하여 최신 데이터를 다시 조회한 뒤 계산한다. 한 번 계산한 값을 캐시하여 재사용하지
않는다.
...
- [ ] 메뉴 표시 시 시료 요약 정보(등록된 시료 종류 수, 전체 재고 수량 합계)가 함께 표시됨,
      시료가 없으면 "등록된 시료가 없습니다." 표시
- [ ] 하위 컨트롤러 실행 후 메인 메뉴로 복귀했을 때, 그 사이 등록/변경된 시료 데이터가 요약
      정보에 최신 상태로 반영됨(매번 새로 조회, 캐시된 값 재사용 금지)
